...
def test_filter(exploreuvl_service):
    with patch.object(exploreuvl_service.repository, 'filter') as mock_filter:
        ...
        tags = []
        advanced_search = exploreuvl_service.filter(query, title, description, authors, q_tags, bytes, publication_type, tags)
        ...
        query = "any"
        normal_search = exploreuvl_service.filter(query, title, description, authors, q_tags, bytes, publication_type, tags)
